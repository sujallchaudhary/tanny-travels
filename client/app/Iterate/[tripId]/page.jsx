...
import { faKitchenSet, faPlane, faTicket } from "@fortawesome/free-solid-svg-icons";
...
const TripPlanner = () => {
  ...
  useEffect(() => {
    const fetchTripData = async () => {
      try {
        const response = await fetch(`http://localhost:4500/iterate/${params.tripId}`);
    ...
    <div className="max-w-3xl mx-auto bg-black text-white">
      ...
      <div className="border border-gray-700 p-2 relative">
        ...
        <div className="mt-5 flex space-x-2 border border-gray-700 p-3 rounded-2xl">
          ...
          <button className="bg-red-600 px-4 py-2 rounded-lg text-white" onClick={() => setFlightVisible(!flightVisible)}>
        ...
        {tripData.modelResponse.map((dayPlan) => (
        <div key={dayPlan.day} className="mt-5">
          ...
          <div className="flex flex-col">
            {dayPlan.activities.map((activity, index) => (
              ...
              >
                <span className="mb-2 text-gray-300 text-sm">{activity.time}</span>
                <div className="flex items-center space-x-2">
                  ...
                  <span className="font-bold text-xl">{activity.activity_name}</span>
                </div>
        ...
        <div>
          <HotelRecommendations hotels={tripData.hotels}/>
        </div>
        <div>
          <TourRecommendation />
        </div>
